...
test "Type Annotation in AbegStatement" {
    ...
    const abeg_stmt = ast.AbegStatement{ .token = createToken(.ABEG, "abeg", 1, 1, "test.zig", "/test/path"), .name = ast.Identifier{ .token = createToken(.IDENT, "age", 1, 1, "test.zig", "/test/path"), .value = "age" }, .value = int_expr, .type_annotation = type_annotation };
    try testing.expect(abeg_stmt.type_annotation != null);
    try testing.expectEqualStrings("int", abeg_stmt.type_annotation.?.name);
    defer allocator.destroy(int_expr);
}
