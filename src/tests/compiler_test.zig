...
fn runCompilerTests(allocator: std.mem.Allocator, test_cases: []const CompilerTestCase) !void {
    for (test_cases) |test_case| {
        ...
        std.debug.print("Parsed program statement length is: {}\n", .{program.statements.items.len});
        std.debug.print("\nDEBUG_COMPILER_TEST AST: {any}\n", .{program.statements.items[0].expression_statement.token.type});
        ...
        std.debug.print("Generated bytecode: {any}\n", .{bytecode});
